...
static DBusHandlerResult request_authorization_message(DBusConnection *conn,
						DBusMessage *msg, void *data)
{
	DBusMessage *reply;
	const char *path;
	...
	if (!dbus_message_get_args(msg, NULL, DBUS_TYPE_OBJECT_PATH, &path,
							DBUS_TYPE_INVALID)) {
		fprintf(stderr, "Invalid arguments for RequestAuthorization method");
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	}
						...
						DBusMessage *msg, void *data)
{
	...
	if (!dbus_message_get_args(msg, NULL, DBUS_TYPE_OBJECT_PATH, &path,
						DBUS_TYPE_UINT32, &passkey,
							DBUS_TYPE_INVALID)) {
		fprintf(stderr, "Invalid arguments for RequestConfirmation method");
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	}
						...
						DBusMessage *msg, void *data)
{
	...
	//
	if (dbus_message_is_method_call(msg, "org.bluez.Agent1",
							"RequestPinCode"))
	{
							...
							"RequestAuthorization"))
	{
		...
		return request_authorization_message(conn, msg, data);
	}
	...
	if (dbus_message_is_method_call(msg, "org.bluez.Agent1", "Cancel"))
	{
		printf("Cancel.\n");
...
static int request_default_agent(DBusConnection *conn, const char *adapter_path,
							const char *agent_path)
{
	DBusMessage *msg, *reply;
	DBusError err;
...
{
	const char *capabilities = "DisplayOnly";
	struct sigaction sa;
	DBusConnection *conn;
	...
	if (request_default_agent(conn, adapter_path, agent_path) < 0) {
		dbus_connection_unref(conn);
		exit(1);
	}
